...
首次部署请阅读 [部署说明：输入命令 → 按提示填写 → 验证成功](docs/部署说明.md)。面向第一次使用本项目的部署者，包含域名解析、安装、后台登录、密码重置、更新和备份。
...
正式域名没有预设。首次部署在自己的 Linux 服务器中依次执行：
...
每一步的命令、实际提示、应该输入的内容和成功结果，见 [逐步部署说明](docs/部署说明.md)。请一次执行一条命令；已有项目不重复克隆。

配置细节、GitHub 上传注意事项、备份和恢复见 [Linux 一键部署指南](docs/Linux一键部署指南.md)。
