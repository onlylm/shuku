...
更新：2026-08-31。入口：项目根目录 `deploy.sh`。
...
GitHub 工作流包含真实 Linux 容器构建、MySQL 迁移、初始化及备份验证。初始发布的 [自动检查记录](https://github.com/onlylm/shuku/actions/runs/33345368224) 已通过；使用其他版本时，应查看对应版本的检查结果。

本次修复旧初始迁移直接读取当前模型、导致新库重复创建后续表的问题：初始迁移现在使用冻结的结构快照。已有数据库不会重跑已应用的迁移，不需要清库或重新生成版本记录。

2026-08-31 已获得一台实际 Linux 服务器部署成功的终端记录：应用启动、HTTPS 证书校验、数据库、前台、后台登录页、robots 与 sitemap 检查通过。该记录不代表所有服务器环境或运营功能均已验收；每个部署者仍需使用自己的域名完成检查。

真实服务器的 MySQL 备份恢复演练、已有书库迁移、CF 封面和真实网盘下载仍需分别验证，不能用部署成功替代这些验收。

Windows 桌面整理软件有独立的测试和已知问题，曾在 `test_quark_directory_and_share_retry` 中出现 `stage_file` 未定义；网站工作流不包含该桌面测试，不能以网站部署成功推断桌面批量上传可用。本次文档更新不修改桌面软件。
