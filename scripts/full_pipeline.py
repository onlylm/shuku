...
def main():
    ...
    ap.add_argument("--source-reference", default="自有/已购电子书资源")
    ...
    ap.add_argument("--force", action="store_true", help="忽略已完成标记，强制重做封面/网盘/网站")
    ap.add_argument("--book-ids", default="", help="逗号分隔的 book_id 列表，只跑这些书（与 --limit 互斥时优先）")
    args = ap.parse_args()
    ...
    opts = dict(provider=args.provider, publish=args.publish, limit=args.limit, batch=args.batch,
                ...
                auto_classify=args.auto_classify, auto_rights=args.auto_rights, force=args.force,
